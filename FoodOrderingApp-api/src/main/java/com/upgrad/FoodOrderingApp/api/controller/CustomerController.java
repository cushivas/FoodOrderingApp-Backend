...
public class CustomerController {
	...
	@Autowired
	Utility utility; // It Provides Data Check methods for various cases

	/*
	 * The method handles Customer SignUp Related request.It takes the details as
	 * per in the SignupCustomerRequest & produces response in
	 * SignupCustomerResponse and returns UUID of newly Created Customer and Success
	 * message else Return error code and error Message.
	 */
	@CrossOrigin
	...
	public ResponseEntity<SignupCustomerResponse> signUpCustomer(
		...
		// Passing the Customer entity to the customerService saveCustomer method to
	...
	/*
	 * This Method handles the Login request and takes authorization parameter in
	 * Base64 coded and produces a LoginResponse containing info customer and
	 * response header containing bearer accessToken. If error returns the error
	 * code with corresponding Message.
	 */
	@CrossOrigin
	...
	public ResponseEntity<LoginResponse> customerLogin(@RequestHeader("authorization") final String authorization)
		...
		// if authenticated it returns CustomerAuthEntity conating the details as
		// required.
		...
		// For CrossOrigin and to let browser access header and set header in
		// sessionStorage
		...
		LoginResponse loginResponse = new LoginResponse().id(customerAuthEntity.getCustomer().getUuid())
				...
				.lastName(customerAuthEntity.getCustomer().getLastName()).message("LOGGED IN SUCCESSFULLY");

		return new ResponseEntity<LoginResponse>(loginResponse, headers, HttpStatus.OK);
	}

	/*
	 * This method handles the customer logout ,It takes the Bearer accessToken from
	 * authorization in the header and logs out the customer and returns a
	 * LogoutResponse conatining UUID of customer and the successful message.If
	 * error returns the error code with corresponding Message.
	 */
	@CrossOrigin
	...
	public ResponseEntity<LogoutResponse> customerLogout(@RequestHeader("authorization") final String authorization)
		...
		// Calls customerService logout method which takes accessToken,if data is all
		// right updates customerAuthEntity logout time and returns updated one.
		CustomerAuthEntity customerAuthEntity = customerService.logout(accessToken);

		// Creating Logout response for the request containing UUID and Successful
		// message.
	...
	/*
	 * This method handles the customer details update request. Takes the request as
	 * UpdateCustomerRequest and produces UpdateCustomerResponse containing the
	 * details of the updated Customer. If error returns the error code with
	...
	public ResponseEntity<UpdateCustomerResponse> updateCustomerDetails(
		...
		utility.isValidUpdateCustomerRequest(updateCustomerRequest.getFirstName());

		// Access the accessToken from the request Header
		String accessToken = authorization.split("Bearer ")[1];

		// Calls customerService getCustomerMethod to check the validity of the
		// customer.this methods returns the customerEntity to be updated.
	...
	/*
	 * This method is used to update password of the customer.It takes
	 * updatePasswordRequest and produces updatePasswordResponse containing UUID and
	 * the successful message. If error returns the error code with corresponding
	 * Message.
	 */
	@CrossOrigin
	...
	public ResponseEntity<UpdatePasswordResponse> updateCustomerPassword(
		...
		utility.isValidUpdatePasswordRequest(updatePasswordRequest.getOldPassword(),
		...
		// updating the customer entity
		CustomerEntity updatedCustomerEntity = customerService.updateCustomerPassword(oldPassword, newPassword,
