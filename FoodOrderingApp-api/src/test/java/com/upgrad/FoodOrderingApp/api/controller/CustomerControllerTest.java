...
public class CustomerControllerTest {
	...
	public void shouldNotSignUpIfTheContactIsAlreadySignedUp() throws Exception {
		when(mockCustomerService.saveCustomer(any())).thenThrow(new SignUpRestrictedException("SGR-001",
	...
	public void shouldNotLogoutIfCustomerIsAlreadyLoggedOut() throws Exception {
		when(mockCustomerService.logout("auth")).thenThrow(new AuthorizationFailedException("ATHR-002",
	...
	public void shouldNotLogoutIfSessionIsExpired() throws Exception {
		when(mockCustomerService.logout("auth")).thenThrow(new AuthorizationFailedException("ATHR-003",
	...
	public void shouldNotUpdateCustomerDetailsWhenCustomerIsNotLoggedIn() throws Exception {
		...
		mockMvc.perform(put("/customer").contentType(MediaType.APPLICATION_JSON_UTF8_VALUE)
				.header("authorization", "Bearer auth").content("{\"first_name\":\"first\", \"last_name\":\"last\"}"))
				.andExpect(status().isForbidden()).andExpect(jsonPath("code").value("ATHR-001"));
		verify(mockCustomerService, times(1)).getCustomer("auth");
		verify(mockCustomerService, times(0)).updateCustomer(any());
	}
	...
	public void shouldUpdateCustomerPassword() throws Exception {
		...
		when(mockCustomerService.getCustomer("auth")).thenReturn(customerEntity);
		when(mockCustomerService.updateCustomerPassword("oldPwd", "newPwd", customerEntity)).thenReturn(customerEntity);
	...
	public void shouldNotUpdateCustomerPasswordIfOldPasswordIsEmpty() throws Exception {
		mockMvc.perform(put("/customer/password").contentType(MediaType.APPLICATION_JSON_UTF8_VALUE)
				...
				.andExpect(status().isBadRequest()).andExpect(jsonPath("code").value("UCR-003"));
		verify(mockCustomerService, times(0)).getCustomer(anyString());
		verify(mockCustomerService, times(0)).updateCustomer(any());
	}
